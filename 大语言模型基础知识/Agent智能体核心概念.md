...
```
***
### 2.什么是 思维链（Chain of Thought, CoT）
#### 2.1 是什么
```java
...
```
***
### 4.什么是 Reflecting反思模式 - 优化AI的输出质量
#### 4.1 是什么
```java
Reflecting 反思模式（通常指 Basic Reflection 或 Reflection Agent）是一种用于提升 AI 智能体（Agent）输出质量的架构范式，其核心理念是通过引入“自我审视”或“独立审查”机制，让模型在生成结果后进行评估与迭代优化。
Reflecting 模式模拟了人类“起草-修改-定稿”的思维过程。它通过两个角色的互动来实现：**生成器（Generator）**负责根据用户指令生成初始回答；**反思器（Reflector）**则负责审查该草稿，指出缺陷、评估质量并提出改进建议。生成器再根据反思器的反馈进行修订，如此循环往复，直到达到预设的质量标准或迭代次数  
```
#### 4.2 工作机制
```java
...
```
***
### 5.什么是短期记忆 - 指模型在当前对话上下文窗口内能够访问和引用的历史信息
#### 5.1 是什么
```java
